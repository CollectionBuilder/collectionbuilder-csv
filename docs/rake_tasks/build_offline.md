...
2. Download external media (images, PDFs, audio) referenced in the "object_location", "image_small", and "image_thumb" fields of your metadata CSV into the "objects" folder of the output directory (not your project's "objects" folder). Downloaded files are named by objectid following CB conventions, e.g. "objects/small/demo_001_sm.jpg". Any download that fails is left pointing to the original external link, and a summary is written to "offline_build_log.txt" in the output directory. Downloads are cached in an "offline_cache" folder in your project, so repeat builds do not fetch everything again.
...
| refresh_downloads | re-download external media even when a copy is already in the "offline_cache" folder. Use when you know the remote files have changed. | false |

Pass options as rake arguments: 

`rake build_offline[false,"my_offline_copy","assets/lib",true,false]`

## Notes

The Jekyll build cleans the output directory, so downloaded media is kept in the "offline_cache" folder between runs and copied back in. The cache is safe to delete at any time; deleting it just means the next build downloads everything again. Both the output directory and the cache are excluded from the Jekyll build automatically. If you use a custom output_dir, you may still want to add it to `exclude` in "_config.yml" and to ".gitignore" so it is not picked up by `jekyll serve` or committed to your repository.
...
If you have external downloads fail, you may want to manually download those images and add them to your project and metadata, then re-run the task. Successful downloads are served from the cache on the next run, so a re-run only retries what failed. 
