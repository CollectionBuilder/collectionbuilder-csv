...
YouTube items are supported in Item pages via the `item_video_embed` "object_template". 
...
The default `item_video_embed` object_template will display the video embedded in an [YouTube Player iframe](https://developers.google.com/youtube/iframe_api_reference). 
