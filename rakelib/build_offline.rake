...
#
# downloaded media is cached in "offline_cache/" so that repeat builds do not re-download
# everything; the cache is safe to delete at any time.
#
...
# directory holding downloaded external media between builds. the Jekyll build cleans the
...
def offline_rewrite_links(content, depth, url_map, top_level = nil, type: :html)
  prefix = '../' * depth
  # paths in HTML and CSS resolve against the file itself; everything else (standalone JS,
  # data exports) gets a root-relative path. used by both the media and sentinel replacements
  # so that a single file never mixes the two styles.
  ...
  # 1. replace downloaded external media URLs with local paths.
  #    longest URL first, so that a URL which is a prefix of another (e.g. the same path with a
  #    query string appended) cannot be substituted inside it and corrupt the longer match.
  url_map.sort_by { |external_url, _| -external_url.length }.each do |external_url, local_path|
    content = content.gsub(external_url, "#{root}#{local_path.delete_prefix('/')}")
  end
...
task :build_offline, [:download_external, :output_dir, :skip_rewrite, :download_video, :refresh_downloads] do |_t, args|
  args.with_defaults(
  ...
  #   - adds the output directory and the download cache to the exclude list so that neither a
  #     custom output_dir nor the cache is ever read back in as site content
  ENV['JEKYLL_ENV'] = 'offline'
  excludes = Array(config['exclude']).map(&:to_s)
  [offline_dir, OFFLINE_CACHE_DIR].each do |dir|
    excludes << "#{dir}/" unless excludes.include?(dir) || excludes.include?("#{dir}/")
  end
  ...
  url_map = {}
  downloaded = []   # urls fetched from the network in this run
  cached = []       # urls served from the download cache
      ...
      unless File.exist?(metadata_file)
        ...
        OFFLINE_MEDIA_FIELDS.each do |field, (subdir, suffix)|
          ...
          csv_data.each do |row|
            ...
            elsif File.exist?(cache_path) && !refresh_downloads
              # the Jekyll build cleans the output directory, so downloads are kept in the cache
              # between runs and copied in rather than fetched again
              FileUtils.cp(cache_path, dest_path)
              cached << url
              url_map[url] = root_relative
            elsif offline_download(url, cache_path)
              FileUtils.cp(cache_path, dest_path)
              downloaded << url
              url_map[url] = root_relative
            else
        ...
        summary << "  reused from cache '#{OFFLINE_CACHE_DIR}/': #{cached.size}"
  ...
  if File.exist?(svg_sprite_path)
    ...
    Dir.glob(File.join(offline_dir, '**', '*.html')).each do |filepath|
      ...
      # the block form keeps the sprite literal; a string replacement would expand any
      # backslash escape it contains as a backreference.
      injected = new_content.sub!(/(<body\b[^>]*>)/) { "#{$1}\n#{inline_sprite}" }

      # rewrite href references that point to the external sprite file to fragment-only hrefs
      # (e.g. href="#icon-image"), which reference the now-inlined symbols. only safe once the
      # sprite is actually present: a page with no <body> tag would otherwise be left with
      # fragment hrefs pointing at symbols that were never injected.
      ...
      if injected
        new_content.gsub!(/(href=["'])[^"']*cb-icons\.svg#/, '\1#')
      elsif content.include?('cb-icons.svg#')
        puts "  Warning: no <body> tag in '#{rel}', leaving its icon references unchanged"
        skipped_pages += 1
      end
    ...
    puts "  #{inlined} file(s) updated with inline SVG sprite.#{skipped_pages.zero? ? '' : " (#{skipped_pages} skipped)"}"
  else
